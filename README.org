...
To trigger the two step export, first build the most recent version of the kih-xds-generator project, by running this command in the kih-xds-generator project:
#+begin_src sh
  make build-container
#+end_src

After this, run this command in this project:
#+begin_src sh
  make start-dev-env
#+end_src

You can then export all measurements to a json file by running:
#+begin_src sh
  go run main.go fetch-measurements --out-file measurements.json
#+end_src

These can later be sent to the generator by running:
#+begin_src sh
  go run main.go process-measurements --in-file measurements.json
#+end_src
